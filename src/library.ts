...
function getDisplayTrackName(track: Track | (Track & { title?: string })): string {
  const n = (track as any).name ?? (track as any).title ?? "Untitled";
...
function getAlbumTotalDurationSeconds(album: Album): number {
  let sum = 0;
  for (const t of album.tracks) sum += t.duration ?? 0;
  return sum;
}

function getUniqueYearsDesc(albums: Album[]): number[] {
  return Array.from(new Set(albums.map(a => a.year))).sort((a, b) => b - a);
}

function getUniqueGenres(albums: Album[]): string[] {
  return Array.from(new Set(albums.map(a => a.genre))).sort((a, b) =>
    a.localeCompare(b)
  );
}

function spotifyAlbumUrl(id: string): string {
...
function renderAlbumCards(albums: Album[]): void {
  ...
  for (const album of albums) {
    ...
    const trackItems = album.tracks
      .map(
        t =>
          `<li><a href="${spotifyTrackUrl(t.id)}" target="_blank" rel="noopener noreferrer">${getDisplayTrackName(
            t
          )}</a> - ${formatSecondsAsMmSs(t.duration)}</li>`
      )
      .join("");
...
function applyFilters(): void {
  ...
  const genreSelect = document.getElementById("genreFilter") as HTMLSelectElement | null;
...
function buildYearFilter(albums: Album[]): void {
  const select = document.getElementById("yearFilter") as HTMLSelectElement | null;
  if (!select) return;
...
function buildGenreFilter(albums: Album[]): void {
  const select = document.getElementById("genreFilter") as HTMLSelectElement | null;
  if (!select) return;
